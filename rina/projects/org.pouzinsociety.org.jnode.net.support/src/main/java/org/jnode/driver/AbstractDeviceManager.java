/*
 * $Id: AbstractDeviceManager.java 4195 2008-06-04 19:38:45Z lsantha $
 *
 * JNode.org
...
public abstract class AbstractDeviceManager implements DeviceManager {
    ...
    protected static final Log logger = LogFactory.getLog(AbstractDeviceManager.class);

    /**
     * Create a new instance
     */
    public AbstractDeviceManager() {
        cmdLine = ""; //(String) new GetPropertyAction("jnode.cmdline", "");
    ...
    public final void register(Device device)
        ...
        if (cmdLine.indexOf("no" + device.getId()) >= 0) {
            logger.info("Blocking the start of " + device.getId());
            shouldStart = false;
        }
        ...
        if (shouldStart) {
            ...
            try {
                ...
                // Should not happen
                logger.error("Device removed before being started", ex);
            }
        }
    }
    ...
    public final void unregister(Device device) throws DriverException {
        ...
        try {
            ...
            // Not found, so stop
            logger.debug("Device not found in unregister");
        }
    }
    ...
    public final void start(Device device) throws DeviceNotFoundException,
        ...
        if (!device.isStarted()) {
            try {
                logger.debug("Starting " + device.getId());
                final StopWatch sw = new StopWatch();
                device.start();
                sw.stop();
                if (sw.isElapsedLongerThen(defaultStartTimeout)) {
                    logger.error("Device startup took " + sw + ": "
                        + device.getId());
                } else if (sw.isElapsedLongerThen(fastStartTimeout)) {
                    logger.info("Device startup took " + sw + ": "
                        + device.getId());
                }
                logger.debug("Started " + device.getId());
            } catch (DriverException ex) {
                logger.error("Cannot start " + device.getId(), ex);
            } catch (Throwable ex) {
                logger.error("Cannot start " + device.getId(), ex);
            }
        }
    ...
    public final void stop(Device device) throws DeviceNotFoundException,
        ...
        if (device.isStarted()) {
            logger.debug("Starting " + device.getId());
            device.stop(false);
            logger.debug("Stopped " + device.getId());
        }
    }
    ...
    public final void stopDevices() {
        while (!devices.isEmpty()) {
            ...
            try {
                logger.debug("Stopping device " + dev.getId());
                unregister(dev);
            } catch (DriverException ex) {
                logger.error("Failed to stop device " + dev.getId(), ex);
            }
        }
            ...
            if (dev.getDriver() == null) {
                ...
                if (drv != null) {
                    try {
                        ...
                        logger.error("Cannot start " + dev.getId(), ex);
                    } catch (DeviceNotFoundException ex) {
                        // Should not happen
                        logger.error("Device is gone before is can be started " + dev.getId(), ex);
                    }
                }
            }
        }
    ...
    protected final void findDevices() throws InterruptedException {
        ...
        for (DeviceFinder finder : finders) {
            try {
                ...
                logger.error("Error while trying to find system devices", ex);
    ...
    protected final Driver findDriver(Device device) {
        synchronized (mappers) {
            for (DeviceToDriverMapper mapper : mappers) {
                ...
                if (drv != null) {
                    logger.debug("Found driver for " + device);
                    return drv;
                }
            }
        }
        logger.debug("No driver found for " + device
    ...
    protected final void fireRegisteredEvent(Device device) {
        ...
        for (DeviceManagerListener l : list) {
            ...
            if (sw.isElapsedLongerThen(100)) {
                logger.error("DeviceManagerListener took " + sw
    ...
    public final void fireStartedEvent(Device device) {
        ...
        for (DeviceListener l : list) {
            ...
            if (sw.isElapsedLongerThen(100)) {
                logger.error("DeviceListener (in manager) took " + sw
