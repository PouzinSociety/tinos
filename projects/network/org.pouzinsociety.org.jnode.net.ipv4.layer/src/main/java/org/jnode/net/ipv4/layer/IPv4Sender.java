/*
 * $Id: IPv4Sender.java 4214 2008-06-08 04:37:59Z crawley $
 *
 * JNode.org
...
public class IPv4Sender implements IPv4Constants, EthernetConstants {
    ...
    private IPv4NetworkLayer ipNetworkLayer;
    ...
    public IPv4Sender(IPv4NetworkLayer ipNetworkLayer) {
    	this.ipNetworkLayer = ipNetworkLayer;
    ...
    /**
     * Generate the corresponding ethernet address for a multicast address.
     * @param dstAddr
     * @return
     */
     private HardwareAddress getMulticastAddress(IPv4Address dstAddr) {
	byte[] bytes = dstAddr.toByteArray();
	return new EthernetAddress((byte) 0x01, (byte) 0x00, (byte) 0x5e,
			(byte) (bytes[1] & 0x7F), bytes[2], bytes[3]);
     }
    ...
    private HardwareAddress findDstHWAddress(IPv4Route route, IPv4Header hdr, SocketBuffer skbuf)
		...
		return getMulticastAddress(hdr.getDestination());    
     ...
     * @return
     */
    private ARPService getARP() {
    	if (arp == null) {
    		arp = ipNetworkLayer.getArp();
    	}
    	return arp;
    }
