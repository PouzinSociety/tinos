...
public final class TCPClient {
	...
	public TCPClient(TransportLayer transportLayer, long endTime)
			throws IOException {
	...
	private byte[] readTCP(Socket s) throws IOException {
		...
		log.debug(hexdump.dump("TCP read", in));
		return (in);
	}

	private void writeTCP(Socket s, byte[] out) throws IOException {
		log.debug(hexdump.dump("TCP write", out));
	...
	byte[] sendAndWait(SocketAddress remote, SocketAddress local, byte[] out)
			throws IOException {
		byte[] in;
		bind(local);
		connect(remote);
