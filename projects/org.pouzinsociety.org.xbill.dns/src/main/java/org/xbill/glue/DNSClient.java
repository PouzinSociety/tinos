 ...
 */
public
DNSClient(String hostname) throws UnknownHostException {
	if (hostname == null) {
 ...
 */
public
DNSClient() throws UnknownHostException {
	this(null);
}
...
send(Message query) throws IOException {
	...
	do {
		...
		if (tcpFlag || out.length > udpSize)
			tcp = true;
		if (tcp)
			in = TCPClient.sendrecv(transportLayer, localAddress, address, out,
						endTime);
		else
			in = UDPClient.sendrecv(transportLayer, localAddress, address, out,
						udpSize, endTime);
...
sendAXFR(Message query) throws IOException {
	...
	ZoneTransferIn xfrin = ZoneTransferIn.newAXFR(qname, transportLayer, address, tsig);
