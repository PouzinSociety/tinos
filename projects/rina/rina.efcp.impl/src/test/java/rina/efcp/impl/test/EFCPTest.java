...
public class EFCPTest {
	...
	private void initConnectionA(){
		...
		connectionId.setDestinationCEPId(new Unsigned(EFCPConstants.PortIdLength, 0x01));
		connectionId.setSourceCEPId(new Unsigned(EFCPConstants.PortIdLength, 0x01));
		connectionId.setQosId(new Unsigned(EFCPConstants.QoSidLength, 0x01));
		...
		connectionA.setSourcePortId(new Unsigned(EFCPConstants.PortIdLength, 0x03));
		connectionA.setDestinationPortId(new Unsigned(EFCPConstants.PortIdLength, 0x04));
	}
	...
	private void initConnectionB(){
		...
		connectionB.setSourcePortId(new Unsigned(EFCPConstants.PortIdLength, 0x04));
		connectionB.setDestinationPortId(new Unsigned(EFCPConstants.PortIdLength, 0x03));
	}
	...
	public void testBasicEFCPBehaviour(){
		...
		PDU pdu = PDU.createPDUFromByteArray(rmt.getPdu());
		...
		Assert.assertArrayEquals(pdu.getFlags().getBytes(), new byte[]{EFCPConstants.completeFlag});
		Assert.assertArrayEquals(pdu.getPduType().getBytes(), new byte[]{0x00});
		Assert.assertEquals(pdu.getPduLength(), EFCPConstants.pciLength + 37);
