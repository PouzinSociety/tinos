...
public class DNSServer{
	...
	private TransportLayer udpTransportLayer = null;
	private TransportLayer tcpTransportLayer = null;
	// Force the ordering
	...
	public DNSServer(){
		log.debug("Created instance of the DNS server, waiting for dependencies to initialize...");
	}
	...
	public void initialize() throws IOException, ZoneTransferException {
		...
		while (stackConfiguration.Complete() == false && i < 100) {
			...
			i++;
			log.info("Waiting for stack configuration completion");
		}
		FileInputStream fs;
		...
		log.debug("Initializing DNS Server...");
		try {
			isr = new InputStreamReader(this.getClass().getResourceAsStream("/dns/dns.conf"));
			br = new BufferedReader(isr);
		}
		catch (Exception e) {
			log.error("Cannot open dns.conf configuration file");
			return;
		}
		...
		try {
			...
			ex.printStackTrace();
			log.error("Problems creating DNS Server");
		}
	}
	...
	public void serveTCP(InetAddress addr, int port) {
		try {
			ServerSocket.setSocketFactory(tcpTransportLayer.getSocketImplFactory());
	...
	public void serveUDP(InetAddress addr, int port) {
		try {
			DatagramSocket.setDatagramSocketImplFactory(udpTransportLayer.getDatagramSocketImplFactory());
